...
static int acquire_azure_cloud_metadata(NetworkCloudManager **ret) {
        ...
        r = azure_acquire_cloud_metadata(&m);
        if (r < 0)
                return r;
...
static int link_dump_azure_cloud_public_ip(const LinkInfo *info, Table *table) {
...
static int link_status_one(
        ...
        (void) link_dump_azure_cloud_public_ip(info, table);
