...
static int mp_tcp_new_static(Network *network, const char *filename, unsigned section_line, MPTCP **ret) {
        ...
        r = ordered_hashmap_ensure_allocated(&network->mp_tcp_by_section, &network_config_hash_ops);
        if (r < 0)
                return r;

        r = ordered_hashmap_put(network->mp_tcp_by_section, mp_tcp->section, mp_tcp);
        if (r < 0)
                return r;
...
int config_parse_mp_tcp_address(
        ...
        r = in_addr_from_string_auto(rvalue, &f, &mp_tcp->address);
        if (r < 0) {
                log_syntax(unit, LOG_WARNING, filename, line, r,
                           "Failed to parse MPTCP '%s=', ignoring assignment: %s", lvalue, rvalue);
                return 0;
...
int config_parse_mp_tcp_uint32(
        ...
        if (isempty(rvalue)) {
                if (streq(lvalue, "LimitSubflows"))
                        network->manager->mp_tcp_subflows = (uint32_t) -1;
                else if (streq(lvalue, "LimitAddress"))
                        network->manager->mp_tcp_add_addr_accepted = (uint32_t) -1;
                else
        ...
        if (streq(lvalue, "LimitSubflows")) {
                network->manager->mp_tcp_subflows = k;
        } else if (streq(lvalue, "LimitAddress"))
                network->manager->mp_tcp_add_addr_accepted = k;
        else
                assert_not_reached("Invalid lvalue");
