...
int dns_stub_extra_new(DNSStubListenerExtra **ret) {
        ...
        *l = (DNSStubListenerExtra) {
                .dns_stub_extra_udp_fd = -1,
                .dns_stub_extra_tcp_fd = -1,
        };
...
static int manager_dns_stub_udp_fd_extra(Manager *m) {
        _cleanup_free_ char *pretty = NULL;
        DNSStubListenerExtra *l;
        Iterator i;
        int r;

        ORDERED_HASHMAP_FOREACH(l, m->dns_extra_stub_listeners, i) {
                _cleanup_close_ int fd = -1;
                socklen_t salen;

                if (l->dns_stub_extra_udp_fd >= 0)
                        continue;
                ...
                r = socket_address_len(&l->address, &salen);
                if (r < 0)
                        goto fail;

                if (bind(fd, &l->address.sockaddr.sa, salen) < 0) {
                        r = -errno;
                        goto fail;
                }

                r = sd_event_add_io(m->event, &l->dns_stub_extra_udp_event_source, fd, EPOLLIN, on_dns_stub_packet, m);
                if (r < 0)
                        goto fail;

                (void) sd_event_source_set_description(l->dns_stub_extra_udp_event_source, "dns-stub-udp-extra");

                r = sockaddr_pretty(&l->address.sockaddr.sa, FAMILY_ADDRESS_SIZE(l->address.sockaddr.sa.sa_family), true, true, &pretty);
                if (r < 0)
                        return r;

                l->dns_stub_extra_udp_fd = TAKE_FD(fd);

                if (DEBUG_LOGGING) {
                        (void) sockaddr_pretty(&l->address.sockaddr.sa, FAMILY_ADDRESS_SIZE(l->address.sockaddr.sa.sa_family), true, true, &pretty);
                        log_debug("Listening on UDP socket %s.", pretty);
                }
        }
       ...
       if (IN_SET(r, -EADDRINUSE, -EPERM)) {
                if (r == -EADDRINUSE)
                        log_warning_errno(r,
                                          ...
                                          "Turning off local DNS stub extra support.", pretty);
                else
                        log_warning_errno(r,
                ...
                return log_error_errno(r, "Failed to listen on UDP socket %s: %m", pretty);
...
static int manager_dns_stub_tcp_fd_extra(Manager *m) {
        _cleanup_free_ char *pretty = NULL;
        DNSStubListenerExtra *l;
        Iterator i;
        int r;
        ...
        ORDERED_HASHMAP_FOREACH(l, m->dns_extra_stub_listeners, i) {
                ...
                r = sd_event_add_io(m->event, &l->dns_stub_extra_tcp_event_source, fd, EPOLLIN, on_dns_stub_packet, m);
                if (r < 0)
                        goto fail;

                (void) sd_event_source_set_description(l->dns_stub_extra_tcp_event_source, "dns-stub-tcp-extra");

                l->dns_stub_extra_tcp_fd = TAKE_FD(fd);

                if (DEBUG_LOGGING) {
                        (void) sockaddr_pretty(&l->address.sockaddr.sa, FAMILY_ADDRESS_SIZE(l->address.sockaddr.sa.sa_family), true, true, &pretty);
                        log_debug("Listening on TCP socket %s.", pretty);
                }
        }
 ...
 fail:
       (void) sockaddr_pretty(&l->address.sockaddr.sa, FAMILY_ADDRESS_SIZE(l->address.sockaddr.sa.sa_family), true, true, &pretty);
        if (IN_SET(r, -EADDRINUSE, -EPERM)) {
                ...
                return log_error_errno(r, "Failed to listen on TCP socket %s: %m", pretty);
...
int manager_dns_stub_start(Manager *m) {
        ...
        if (ordered_hashmap_size(m->dns_extra_stub_listeners) > 0) {
                log_debug("Creating stub listener extra using %s.",
                          m->dns_stub_listener_mode == DNS_STUB_LISTENER_UDP ? "UDP" :
                          m->dns_stub_listener_mode == DNS_STUB_LISTENER_TCP ? "TCP" :
                          "UDP/TCP");

                if (r >= 0 && IN_SET(m->dns_stub_listener_extra_mode, DNS_STUB_LISTENER_YES, DNS_STUB_LISTENER_UDP))
                        r = manager_dns_stub_udp_fd_extra(m);

                if (r >= 0 && IN_SET(m->dns_stub_listener_extra_mode, DNS_STUB_LISTENER_YES, DNS_STUB_LISTENER_TCP))
                        (void) manager_dns_stub_tcp_fd_extra(m);
        }
...
void manager_dns_stub_stop_extra(Manager *m) {
        ...
        ORDERED_HASHMAP_FOREACH(l, m->dns_extra_stub_listeners, i) {
                l->dns_stub_extra_udp_event_source = sd_event_source_unref(l->dns_stub_extra_udp_event_source);
                l->dns_stub_extra_tcp_event_source = sd_event_source_unref(l->dns_stub_extra_tcp_event_source);

                l->dns_stub_extra_udp_fd = safe_close(l->dns_stub_extra_udp_fd);
                l->dns_stub_extra_tcp_fd = safe_close(l->dns_stub_extra_tcp_fd);
        }
}
