...
#define ntske_packet_reset_parse(p) (p->offset = 0)

DEFINE_TRIVIAL_CLEANUP_FUNC(NTSKEPacket*, ntske_packet_free);

int ntske_append_record(NTSKEPacket *message, uint16_t record_type, const void *data, size_t data_length, bool critical);
int ntske_read_record(NTSKEPacket *message, uint16_t *record_type, uint8_t **data, size_t *data_length, bool *critical);
